  ...
  return (
    <>
      <Flex direction="row" wrap="nowrap" className="array-item" mt={2} mr={5} style={tagPaddingLeftStyle}>
        <Input key="Items" isDisabled value="Items" size="sm" flexShrink={1} margin={2} variant="outline" />
        ...
        />
        <Tooltip hasArrow aria-label="Advanced Settings" label="Advanced Settings" placement="top">
          <IconButton
            isRound
            isDisabled={isReadOnlyState.value}
            size="sm"
        ...
        {state.type.value === "object" && (
          <Tooltip hasArrow aria-label="Add Child Node" label="Add Child Node" placement="top">
            <IconButton
              isRound
              isDisabled={isReadOnlyState.value}
              size="sm"
      ...
      {state.type?.value === "object" && <SchemaObject isReadOnly={isReadOnlyState} schemaState={state} />}
      {state.type?.value === "array" && <SchemaArray isReadOnly={isReadOnlyState} schemaState={state} />}
      <Modal isOpen={localState.isAdvancedOpen.get()} finalFocusRef={focusRef} size="lg" onClose={onCloseAdvanced}>
        <ModalOverlay />
        <ModalContent>
          ...
          <ModalFooter>
            <Button colorScheme="blue" variant="ghost" mr={3} onClick={onCloseAdvanced}>
              Close
            </Button>
          </ModalFooter>
        </ModalContent>
